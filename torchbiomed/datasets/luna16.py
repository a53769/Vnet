...
def load_image(root, series):
    ...
    #fix length
    step = z/IN_LENGHT
    index = np.round(np.arange(0,z,step)).astype(int)
    img = img[index]
...
class LUNA16(data.Dataset):
    ...
    def __getitem_dev(self, index):
        ...
        target = load_mask(self.targets, series)
        ...
        imgs = image.astype(np.float32)
        # img = transforms.ToPILImage()(image)
        if self.transform is not None:
            for index, img in enumerate(imgs):
                img = img.reshape(ye-ys, xe-xs,1)
                imgs[index] = self.transform(img)
            # imgs = self.transform(imgs)
        if self.target_transform is not None:
            target = self.target_transform(target)
        if self.co_transform is not None:
            imgs, target = self.co_transform(imgs, target)
